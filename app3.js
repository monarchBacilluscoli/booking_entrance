...
app.get('/reservation_get', function (req, res) {
    ...
    if (warningString) { // 如果存在sessionCheck不通过（直接URL/页面停留过长），那么将存在提示字符串，并退回登陆页面
        ...
        // console.log(typeof(response.TTB));
        ...
        connection.query('BEGIN', function (err, result) { // 开始事务
            if (err) {
                ...
                connection.query(room, [response.TTB, response.TTD, response.NSB, response.NSD, response.BHB, response.CIN, response.COUT], function (err_r, result_r) {
                    if (err_r) {
                        console.log(err_r); // TODO：这里有一个减法超过无符号整形范围的错误，利用一下就不自己写代码了。
                        connection.query('ROLLBACK'); // 事务回滚
                        ...
                        connection.query(order, response.pin, function (err_o, result_o) {
                            if (err_o || (!result_o.insertId)) { // 如果insertId不存在也不行
                                console.log(err_o);
                                connection.query('ROLLBACK'); // 事务回滚
                                res.send('Sorry, order processing error.')
                                return;
                            } else {
                                // 处理订单下属房间条目
                                var r_rooms = 'INSERT INTO r_room (type,order_id,checkin,checkout) values (?,?,?,?)';
                                for (let i = 0; i < response.TTB; i++) {
                                    connection.query(r_rooms, [1, result_o.insertId, response.CIN, response.COUT], function (err, result) {
                                        if (err) {
                                            console.log(err);
                                            connection.query('ROLLBACK'); // 事务回滚
                                            return;
                                ...
                                for (let i = 0; i < response.TTD; i++) {
                                    connection.query(r_rooms, [2, result_o.insertId, response.CIN, response.COUT], function (err, result) {
                                        if (err) {
                                            console.log(err);
                                            connection.query('ROLLBACK'); // 事务回滚
                                            return;
                                ...
                                for (let i = 0; i < response.NSB; i++) {
                                    connection.query(r_rooms, [3, result_o.insertId, response.CIN, response.COUT], function (err, result) {
                                        if (err) {
                                            console.log(err);
                                            connection.query('ROLLBACK'); // 事务回滚
                                            return;
                                ...
                                for (let i = 0; i < response.NSD; i++) {
                                    connection.query(r_rooms, [4, result_o.insertId, response.CIN, response.COUT], function (err, result) {
                                        if (err) {
                                            console.log(err);
                                            connection.query('ROLLBACK'); // 事务回滚
                                            // 取消事务
                                ...
                                for (let i = 0; i < response.BHB; i++) {
                                    connection.query(r_rooms, [5, result_o.insertId, response.CIN, response.COUT], function (err, result) {
                                        if (err) {
                                            console.log(err);
                                            connection.query('ROLLBACK'); // 事务回滚
                                            // 取消事务
                                ...
                                connection.query(userInfo, [response.phone, response.email, response.pin], function (err_u, result_u) {
                                    if (err_u) {
                                        consol.log(err_u);
                                        res.send('Sorry, personal information update error.');
                                        connection.query('ROLLBACK'); // 事务回滚
                                        return;
                                    } else {
                                        connection.query('COMMIT'); // 事务确认
                                        console.log('-----------------------Order_uploaded----------------------');
                                        console.log(result_u);
                                        console.log("Order time: " + new Date(Date.now())); // 输出时间
                                        console.log("Clien info: " + JSON.stringify(req.headers));
                                        console.log("IP: " + getClientIp(req));
                                        console.log('-----------------------------------------------------------\n\n');
                                        // 记得发送邮件
                                        ...
                                        var placeholder = {
                                            ...
                                            BHB: response.BHB ? '百花2号标间' + response.BHB + '间、' : '\b',
                                        ...
                                        transporter.sendMail(mailOptions, function (err_m) {
                                            if (err_m) {
                                                ...
                                                res.send({
                                                    title: 'Reminder',
                                                    text: 'Payment reminder email sent successfully, please check it in your email:${response.email}, and follow the prompts',
